...
fn process_block_cut_through() -> Result<(), chain::Error> {
	...
	// The input is coinbase and the output is plain, so verify_cut_through() treats them (incorrectly) as different.
	...
	// Transaction itself is treated as valid.
	// Note: verify_cut_through() *should* identify this as invalid.
	let verifier_cache = Arc::new(RwLock::new(LruVerifierCache::new()));
	tx.validate(Weighting::AsTransaction, verifier_cache.clone())
		.expect("tx is valid");

	// But transaction is invalid when verified against the utxo (duplicate commitment).
	assert_eq!(
		chain.validate_tx(&tx).map_err(|e| e.kind()),
		Err(chain::ErrorKind::DuplicateCommitment(commit)),
	);

	// Build a block with this single transaction.
	let block = build_block(&chain, &keychain, &[tx.clone()], true)?;

	// The block is treated as valid.
	// Note: verify_cut_through() *should* identify this as invalid.
	let prev = chain.head_header()?;
	block
		.validate(&prev.total_kernel_offset(), verifier_cache)
	...
	// We should see a DEBUG log msg explaining the block was rejected -
	//     DEBUG grin_chain::txhashset::txhashset - Error returned, discarding txhashset extension:
	//     Duplicate Commitment: Commitment(0846d3e87e60bef4d9e98544939f9e57f8704f6dc0e2e1fd8204495c2dba80368d)
	...
	// without it being wrapped as above.
	{
		...
		assert_eq!(res, Err(chain::ErrorKind::DuplicateCommitment(commit)));
	}
