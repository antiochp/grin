...
fn test_coinbase_maturity() {
...

	{
		...
		chain
			.process_block(block.clone(), chain::Options::NONE)
			.unwrap();
		...
		for _ in 0..3 {
			...
			for _ in 0..3 {
				...
				chain.process_block(block, chain::Options::NONE).unwrap();
			}
			...
			let result = chain.process_block(block, chain::Options::NONE);
			match result {
				Ok(_) => (),
