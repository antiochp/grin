...
pub fn sync_block_headers(
	...
	let sync_head = Tip::from_header(&ctx.batch.get_block_header(&ctx.header_pmmr.head_hash()?)?);
...
pub fn process_block_header(header: &BlockHeader, ctx: &mut BlockContext<'_>) -> Result<(), Error> {
	...
	let header_head = Tip::from_header(&ctx.batch.get_block_header(&ctx.header_pmmr.head_hash()?)?);
